...
- Name: Pageobjcmds.test.ts >> Page object commands
- Location: e2e\Pageobjcmds.test.ts:3:5
...
```ts
  ...
  2  | 
  3  | test("Page object commands", async({page})=>{
  4  |     await page.goto("https://tutorialsninja.com/demo/index.php?route=account/login")
  5  |     const email = page.getByRole('textbox', { name: /email/i });
> 6  |     await email.fill("muhindhar271@gmail.com");
     |                 ^ TimeoutError: locator.fill: Timeout 10000ms exceeded.
  7  |     const pass = await page.getByRole('textbox',{name:'password'});
  8  |     pass.type("12345678");
  9  |     const login = await page.locator("//input[@value='Login']")
  10 |     login.click();
  11 |     //await expect(page.locator("//h2[normalize-space()='My Account']")).toBeVisible();
  12 |     await expect(page.getByText('My Account')).toBeVisible();
  13 | });
```